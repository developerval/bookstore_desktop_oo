...
class Window(object):
    ...
    def __init__(self, window):
        ...
        self.window.title("Book Store Management")
        ...
        b2 = Button(window, text="Search Entry",
                    width=12, command=self.search_entry)
        b2.grid(row=3, column=3)

        b3 = Button(window, text="Add Entry", width=12, command=self.add_entry)
    ...
    def delete_record(self):
        db.delete(self.selected_row[0])
        self.view_command()
        self.clear_entries()

    def update_record(self):
        db.update(self.selected_row[0], self.title_text.get(), self.author_text.get(),
                  self.year_text.get(), self.isbn_text.get())
        self.clear_entries()
        self.view_command()
